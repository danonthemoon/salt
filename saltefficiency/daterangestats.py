...
if __name__=='__main__':
  ...
  # password=os.environ['SDBPASS']
   ...
   if nights == 0:
       ...
       rss_stats = {}
       rss_stats.update({'4. Slew' : median(rss_slewtimes)})
       rss_stats.update({'3. Tracker Slew' : median(rss_trslewtimes)})
       rss_stats.update({'2. Target Acquisition': median(rss_targetacqtimes)})
       rss_stats.update({'1. Instrument Acquisition': median(rss_instracqtimes)})
       #rss_stats.update({'Science Track': median(rss_scitracktimes)})
       hrs_stats = {}
       hrs_stats.update({'1. Instrument Acquisition': median(hrs_instracqtimes)})
       hrs_stats.update({'2. Target Acquisition': median(hrs_targetacqtimes)})
       hrs_stats.update({'3. Tracker Slew' : median(hrs_trslewtimes)})
       hrs_stats.update({'4. Slew' : median(hrs_slewtimes)})
